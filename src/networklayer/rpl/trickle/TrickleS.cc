...
void TrickleS::initialize(int stage)
{
    suppressedDIOs = 0 ;

    EV << "[TrickleS] " << redundancy << endl;

    TrickleNormal::initialize(stage);
}

void TrickleS::intervalEnded(){
    ...
    EV << "TrickleS: firing interval " << curInt / (pow(2, suppressedDIOs + 1) ) << ", " <<  curInt / (pow(2, suppressedDIOs)) << endl;

    // Signal EC
    ec->intervalBegin(curInt);
}

void TrickleS::intervalBegin(){

}

void TrickleS::messageFired() {
...
void TrickleS::initializeTrickle(){
    suppressedDIOs = 0 ;
